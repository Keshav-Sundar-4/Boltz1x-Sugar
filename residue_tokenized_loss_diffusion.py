...
def Linkage_Loss(
    ...
    """
    Calculates a distance-based loss for glycosylation bonds using precise
    atom extraction via raw site features, decoupling it from the token index representation.
    """
    ...
    for b in range(B_orig):
        ...
        for site_data_tensor in sites_tensor_b:
            ...
            p_name = _decode_int_to_str(site_data_tensor[2:6]).upper()
            ...
            p_atom_index = -1
            for p_idx in p_atoms_in_res:
                decoded_name = _decode_one_hot_to_str(ref_name_chars[p_idx]).upper()
                if decoded_name == p_name:
                    p_atom_index = p_idx.item()
                    break
            ...
            if p_atom_index == -1 or g_atom_index == -1:
                continue

            batch_start = b * multiplicity
            batch_end = (b + 1) * multiplicity

            curr_pred_p = pred_coords[batch_start:batch_end, p_atom_index, :]
            curr_pred_g = pred_coords[batch_start:batch_end, g_atom_index, :]
            curr_true_p = true_coords[batch_start:batch_end, p_atom_index, :]
            curr_true_g = true_coords[batch_start:batch_end, g_atom_index, :]

            true_dist = torch.linalg.norm(curr_true_p - curr_true_g, dim=-1)
            pred_dist = torch.linalg.norm(curr_pred_p - curr_pred_g, dim=-1)

            per_bond_loss = (pred_dist - true_dist) ** 2
            
            curr_weights = loss_weights[batch_start:batch_end]
            weighted_site_loss = (per_bond_loss * curr_weights).mean()
            
            total_loss += weighted_site_loss
            site_count += 1

    if site_count == 0:
        return None

    return total_loss / site_count
